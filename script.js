...
function addStudentsCreated(studentsData) {
    try {
        if (!studentsData)
            throw new Error('You can`t access to students data');
        ...
        if (!renderStudents)
            throw new Error('There is a problem finding the function renderStudents');
...
var handleSubmit = function (ev) {
    ...
    try {
        ...
        if (!addNewStudent)
            throw new Error('There is a problem finding the function addNewStudent');
        addNewStudent(student);
        alert('User uploaded successfully');
        if (!modalUpload)
            throw new Error('Can`t find modalUpload from the HTML');
...
var handleEdit = function (ev) {
    ...
    try {
        ...
        var date = ev.target.elements.date.value;
        var edit_1 = students.find(function (element) { return element.id === studentIdEdit; });
...
function handleFilter() {
    try {
        ...
        if (!subject_1)
            throw new Error('Can`t access to subject in filter');
        ...
        if (subject_1.value === '-' && status_1.value !== '-') {
            ...
            var studentsFiltered = students.filter(function (element) { return (element.subject === subject_1.value); });
            checkFilters(studentsFiltered);
        }
        else if ((status_1.value === '-' && subject_1.value === '-')) {
            renderStudents(students);
            ...
            var studentsFiltered = students.filter(function (element) { return ((element.subject === subject_1.value) && (element.status === status_1.value)); });
            checkFilters(studentsFiltered);
        }
        ;
    }
...
try {
    ...
    if (!sortName)
        throw new Error('There is a problem sorting the name');
    var sortLast = document.querySelector("#tblData__titles--lastname");
    if (!sortLast)
        throw new Error('There is a problem sorting the lastname');
    var sortEmail = document.querySelector("#tblData__titles--email");
    if (!sortEmail)
        throw new Error('There is a problem sorting the email');
    var sortDate = document.querySelector("#tblData__titles--date");
    if (!sortDate)
        throw new Error('There is a problem sorting the date');
    var sortSubject = document.querySelector("#tblData__titles--subject");
    if (!sortSubject)
        throw new Error('There is a problem sorting the subject');
    ...
    sortName.addEventListener('click', function () {
        if (order === true) {
            ...
            var sortByFirstName = students.sort(function (a, b) { return b.firstname.localeCompare(a.firstname); });
            renderStudents(sortByFirstName);
            order = true;
        }
        ;
    ...
    sortLast.addEventListener('click', function () {
        if (order === true) {
            var sortByLastName = students.sort(function (a, b) { return a.lastname.localeCompare(b.lastname); });
            renderStudents(sortByLastName);
            order = false;
        }
        else if (order === false) {
            var sortByLastName = students.sort(function (a, b) { return b.lastname.localeCompare(a.lastname); });
            renderStudents(sortByLastName);
            order = true;
        }
        ;
    ...
    sortEmail.addEventListener('click', function () {
        if (order === true) {
            var sortByEmail = students.sort(function (a, b) { return a.email.localeCompare(b.email); });
            renderStudents(sortByEmail);
            order = false;
        }
        else if (order === false) {
            var sortByEmail = students.sort(function (a, b) { return b.email.localeCompare(a.email); });
            renderStudents(sortByEmail);
            order = true;
        }
        ;
    ...
    sortSubject.addEventListener('click', function () {
        if (order === true) {
            var sortBySubject = students.sort(function (a, b) { return a.subject.localeCompare(b.subject); });
            renderStudents(sortBySubject);
            order = false;
        }
        else if (order === false) {
            var sortBySubject = students.sort(function (a, b) { return b.subject.localeCompare(a.subject); });
            renderStudents(sortBySubject);
            order = true;
        }
        ;
    ...
    sortStatus.addEventListener('click', function () {
        if (order === true) {
            var sortByStatus = students.sort(function (a, b) { return a.status.localeCompare(b.status); });
            renderStudents(sortByStatus);
            order = false;
        }
        else if (order === false) {
            var sortByStatus = students.sort(function (a, b) { return b.status.localeCompare(a.status); });
            renderStudents(sortByStatus);
            order = true;
        }
        ;
...
var compareDate = function (firstDate, secondDate) {
    try {
...
var compareDateOposite = function (firstDate, secondDate) {
    try {
        var date1 = new Date(firstDate.date).getTime();
        var date2 = new Date(secondDate.date).getTime();
        return date1 < date2 ? 1 : -1;
...
try {
    document.querySelector('#inputExcel').addEventListener("click", function () {
...
var selectedFile;
try {
    document.querySelector('#inputExcel').addEventListener("change", function (event) {
        selectedFile = event.target.files[0];
        var option = confirm('Are you sure do you want to import this file?');
        if (option === true) {
            var data = [];
            ...
            if (selectedFile) {
                ...
                fileReader.onload = function (event) {
                    ...
                    workbook.SheetNames.forEach(function (sheet) {
                        ...
                        var info = JSON.stringify(rowObject, undefined, 4);
                        var newArrayStudents = JSON.parse(info);
